  ...
  class CompilationStream extends Transform {
    ...
    _flush(cb) {
      ...
      options.jsCode = (options.jsCode || []).concat(this.fileList_.map(file => {
        ...
        console.info('got path', out.path, 'from', file.path);
      ...
      const log = o => {
        // TODO(samthor): This isn't a very good logger for warnings or errors.
        logger(gutil.colors.yellow(this.pluginName_) + `: ${o.file} (${o.type}) ${o.description}`)
      }

      output.warnings.forEach(log);
      output.errors.forEach(log);
      ...
      const file = new gutil.File({
        path: options.jsOutputFile || DEFAULT_OUTPUT_PATH,
